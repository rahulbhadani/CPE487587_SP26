...
\section{\faCheckSquare\enspace {--} True / False}
...
is fully differentiable.
  \\  \TF 
    ...
    \item  Is it possible to implement a neural network in PyTorch without inheriting from \texttt{nn.Module}?
   \\ \TF
...
\\    \TF
 ...
 \\   \TF
    ...
    \item Predicting the glucose level given patients' health metrics is a regression problem. \\ \TF
...
\item The Coefficient of determination $R^2$ can also be applied to deep neural network-based machine learning models. \\ \TF
...
% ============================================================
\section{\faListOl\enspace \textbf{--} Multiple Choice}
...
\item 400
    ...
    \item Which of the following is not an assumption in the Simple Linear Regression model?
...
\item Which of the loss functions is not suitable for regression problems?
...
What is the size of the Jacobian $J$?
\begin{enumerate}
\item  $4 \times 4$
\item $ 4\times 3$
\item $ 3\times 4$
...
% ============================================================
\section{\faMarker\enspace \textbf{--} Short Answer}
...
\subsection{}
Why does a feedforward fully connected neural network not work well with sequential data? \pts{5} 
...
\subsection{}
What is the Markov assumption in an n-Gram Language Model? \pts{5} 
...
% ============================================================
\section{\faCalculator\enspace \textbf{--}  Mathematically Trained}
...
\end{align*}
 ...
 Consider a padding of 1 and a stride of 1, and compute the output after applying the kernel to the input feature matrix.
...
\section{\faCalculator\enspace \textbf{--}  Forward Pass}
...
\section{\faAtom\enspace \textbf{--} Magical CNN}
...
% ============================================================
\newpage
\section{\faCode\enspace \textbf{--} Code Identification}
...
\newpage
\section{\faCode\enspace \textbf{--} Fancy Layers}
...
Provide an implementation for a composite layer \texttt{VGGBlock} representing the above VGG Block implemented in PyTorch by inheriting from \texttt{nn.Module} class that specifies $v$ copies of a $3\times 3$ Conv Layer of padding 1 sequentially connected.
...
\section{\faKeyboard\enspace \textbf{--} Sentence Probability}
...
\section{\faKeyboard\enspace \textbf{--} Embeddings}

Consider that after a particular epoch during the training embedding matrix looks like
...
\item What is the size of the vocabulary?
\item Write the one-hot encoding matrix corresponding to the vocabulary.
\item Assuming that sequences ID assigned to tokens in the vocabulary start from 1, if a particular token sequence has been represented by $ T = \{4, 2, 1, 5, 3, 4 \}$, what is the embedding for the given sequence?
\end{enumerate}
...
\section{\faKeyboard\enspace \textbf{--} Training Master}
...
\textit{Identify different activation functions based on their derivatives.}
...
\section{\faKeyboard\enspace \textbf{--} Code Writing}
...
% \underline{\hspace{\linewidth}}
    ...
    If you need to implement weight initialization for Conv1, how would you do it, and when should the weight initialization function be called?
...
\section{\faKeyboard\enspace \textbf{--} Jacobian and Derivatives}
...
\item Consider $ y = f(x) = e^{x^2 - 1}$. Introduce intermediate variables,  compute intermediate derivatives, and apply the chain rule to determine $\frac{dy}{dx}$.
