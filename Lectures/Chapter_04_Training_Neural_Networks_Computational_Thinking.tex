...
That was the case of one sample; if we consider $n$ samples, then?
...
The process of filling missing values or correcting known errors is called \textbf{imputation}.
...
incomplete variables, which are unknown to us, even after conditioning on observed information
...
\item Get rid of features where \# missing values exceed certain numbers (or percentage)
...
\begin{tblock}{}
Mean and median approaches or anything similar would fail to quantify uncertainty in the estimated missing value.
\end{tblock}
...
\item Compute prior distribution over unknown parameters.
...
\item Draw samples using Markov Chain Monte Carlo simulation that will have plausible missing values.
...
Calculating the posterior is hard, so we use Monte Carlo methods (Markov Chain Monte Carlo (MCMC)).
...
Otherwise, aligning the entire probability distribution of adjusted values is \textbf{Normalization}.
...
\begin{frame}{Batch Size}
    ...
    Determining a suitable batch size is an important consideration in the training process, as it may influence the learning rate of the model. 
...
\begin{frame}{Stochastic Gradient Descent and Batch Size}
    ...
    \Large 
    {\faQuestionCircle~~ What's the problem if we use the entire training set for gradient descent? }
    ...
    \begin{tblock}{}
        Stochastic gradient descent provides an approximation of true gradients using one sample at a time after shuffling the training sample and picking one training sample to update the weights. It optionally adds a small noise to the gradient calculation.
    \end{tblock}
...
\begin{itemize}
    \item The central idea behind {\em\color{MediumBlue}dropout} is to randomly dropout nodes in the neural network with a probability $p$. After the nodes are ``dropped out'', the remaining network is a subnetwork of the original. 
...
\begin{frame}{Regularization in Neural Network}
    ...
    \begin{enumerate}
            ...
            \item L2 regularization shrinks weights, but some of the components of weights are larger than others, which are contributing more to the model.
        \end{enumerate}
...
If a multilayer NN can model $\mathcal{H}$, then it may also be able to model residual $\mathcal{H}(\xbf) - \xbf$, assuming input/output have the same dimensions. So rather than expecting layers to model $\mathcal{H}$, we expect them to model $\Fcal(\xbf) = \mathcal{H}(\xbf) - \xbf$, and the using a skip connection, we have $\Fcal(\xbf) + \xbf$ later.
\end{figure}
...
\item How does the performance change as we increase the number of layers?
...
Usually adhoc, but we can establish some rules. Use a validation set for early stopping.
...
\begin{align*}
\Lcal_{opt}(t) := \min_{t' \leq t} \Lcal_{val}(t')
\end{align*}
...
It means we define generalization loss at epoch $t$ as the relative increase of validation error over the minimum so far. And, we specify a stopping criterion as \textbf{stop when generalization loss after epoch $t$ exceeds a certain threshold $\alpha$.}
...
To avoid stopping while the model is still learning rapidly, we define \textbf{training progress} over a strip of length $k$:
    ...
    \begin{align*}
        P_k(t) &:= 1000 \cdot \left( \frac{\sum_{t'=t-k+1}^{t} \Lcal_{tr}(t')}{k \cdot \min_{t'=t-k+1}^{t} \Lcal_{tr}(t')} - 1 \right)
    \end{align*}
...
\begin{frame}{Stopping Criteria: Successive Increases}
    ...
    \begin{align*}
        ...
        &\Lcal_{va}(t) > \Lcal_{va}(t - k) \\
        UP_1 &: \text{stop after first end-of-strip epoch } t \text{ with } \Lcal_{va}(t) > \Lcal_{va}(t - k)
    \end{align*}
...
\begin{frame}{Advantages of the UP Criteria}
    ...
    But remember, this is just one kind of early stopping example; you can design your own.
...
\section{Customization of Neural Network Architecture}

\begin{frame}
    \sectionpage
\end{frame}

\begin{frame}{\texttt{nn.Module} in PyTorch}

\texttt{nn.Module}: The fundamental building block of PyTorch neural networks.

\begin{block}{Basic structure}

\begin{enumerate}
\item Abstract class
\item A layer or collection of layers
\item Encapsulates state (parameters) and behavior (operations).
\item Can contain another \texttt{nn.Module} object.
\end{enumerate}
...
\begin{center}
\begin{block}{Everything is a Module}

PyTorch follows the principle that neural networks should be composable from simple, reusable components. Whether it's a single linear layer or a 1000-layer ResNet, both are \texttt{nn.Module} instances.
...
\begin{frame}{Philosophy Behind \texttt{nn.Module}}

    \footnotesize

    \begin{block}{Design Principles}
\begin{itemize}
    \item \textbf{Everything is a Module}: From single layer to 1000-layer ResNet
    \item \textbf{Separation of Definition and Execution}:
    \begin{align*}
        \text{Definition} &\neq \text{Execution} \\
        \text{Graph structure} &\neq \text{Forward pass}
    \end{align*}
    \item \textbf{Explicit Over Implicit}: Manual forward pass definition
    \item \textbf{Pythonic Design}: Uses standard Python inheritance and conventions
    \item \textbf{Minimal Abstraction}: Can drop to raw tensor operations when needed
\end{itemize}

\vspace{5pt}

\begin{align*}
\texttt{nn.Module} &= \text{Container} + \text{Behavior} + \text{State} \\
\text{Parameters} &\in \texttt{nn.Parameter} \subset \text{Tensor} \\
\text{Sub-modules} &\in \texttt{nn.Module}
\end{align*}
\end{block}
\end{frame}
...
\begin{frame}{Philosophy Behind \texttt{nn.Module}}
    ...
    \begin{enumerate}
        \item \textbf{Modularity:} Each nn.Module can be a small, reusable component. Complex networks are built by composing these modules. \vspace{2pt} \hrule
        \item {\color{MediumRed}\textbf{Encapsulation:} nn.Module encapsulates both the parameters (as nn.Parameter) and the operations (in the forward method). It also manages the state of the module, including training mode (train/eval) and device placement (CPU/GPU). \vspace{2pt} \hrule }
        \item \textbf{Automatic Differentiation:} By defining the forward pass, the backward pass (gradients) is automatically handled by PyTorch's autograd engine, provided that the operations used in the forward pass are differentiable and track gradients. \vspace{2pt} \hrule

        \item {\color{MediumRed}\textbf{Hierarchical Structure:} nn.Module can contain other nn.Module instances, allowing for tree-like structures. This is how complex networks are built from simpler ones.\vspace{2pt} \hrule}

        \item \textbf{Parameter Management:} nn.Module automatically tracks all nn.Parameter objects that are assigned as attributes. It also provides methods for moving parameters to different devices, saving and loading, etc.\vspace{2pt} \hrule

        \item {\color{MediumRed}\textbf{Hooks:} nn.Module allows for the registration of hooks, which are functions that can be called at various stages (before/after forward/backward). This is useful for debugging, visualization, and extending functionality.}
    \end{enumerate}
    
\end{frame}

\begin{frame}[fragile]{A Class Diagram}
...
Forward is defined in \texttt{forward} function of \texttt{nn.Module} that has to be implemented for the forward propagation structure.
...
\begin{frame}[fragile]{Parameters in \texttt{nn.Module}}

The workflow of creating a Neural Network object for training must register parameters that need to be optimized.

\bigskip

Parameter registration answers the question: \textbf{which parameters to optimize}

\bigskip

as, there can be more attributes in a nn.Module class that won't be optimized.
...
\begin{enumerate}
\item Only registered parameters are saved/loaded
\item Registered tensors move together with \texttt{.to(device)}
\item Only registered parameters get gradients
\end{enumerate}
...
\begin{frame}[fragile]{How Parameter Registration Works Internally}
\begin{block}{Using \texttt{\_\_setattr\_\_}}
\begin{minted}[
    linenos,
    frame=lines,
    framesep=2mm,
...
\begin{block}{Parameter Types}

    \vspace{-15pt}

\begin{align*}
&\text{Parameters} &&\text{→ Learned via gradient descent} \\
&\text{Buffers} &&\text{→ Saved but not learned} %\\
%&\text{Computation} &&\text{→ Temporary values}
\end{align*}
\end{block}
\end{frame}


\begin{frame}{What REALLY happens when you do \texttt{model(x)}?}
...
\begin{tikzpicture}[
    ...
    \node[box] (step1) {1. model.\\\texttt{\_\_call\_\_}(x)};
    \node[box, right=of step1] (step2) {2. Forward\\Pre-Hooks};
    \node[box, right=of step2] (step3) {3. Check JIT\\Tracing ( different path if true)};
    \node[box, right=of step3] (step4) {4. Enter Autograd\\Context};
    \node[box, right=of step4] (step5) {5. model.forward\\+ Graph};

    % --- SECOND ROW: Right to Left (below step 5) ---
    \node[box, below=1.2cm of step5] (step6) {6. Forward\\Hooks Executed};
    \node[box, left=of step6] (step7) {7. Backward Hooks\\Registered};
    \node[box, left=of step7] (step8) {8. Return Result\\with grad\_fn};
    \node[box, left=of step8] (step9) {9. Autograd Graph\\Ready};
    ...
    % Connection between rows (U-turn)
    ...
    % --- Annotation ---
    \node[draw=gray, dashed, fill=yellow!10, rounded corners, font=\footnotesize, 
            text width=12cm, below=1.5cm of step8] (note) 
            {\begin{enumerate}
            \item Submodules recursively call
            \item \texttt{\_\_call\_\_}, operations create Function nodes
            \item Builds computation graph
            \end{enumerate} };
    
    \draw[draw=gray, dashed] (step5.south) -- (note.north west);
...
\begin{frame}{Layers in Neural Network Construction}

    \begin{block}{}
        Inherits \texttt{nn.Module}
...
\begin{frame}[fragile]{Some Custom Layers for Feature Engineering}
...
\begin{frame}[fragile]{Some Custom Layers for Feature Engineering}
    ...
    \begin{columns}
        ...
        class ZScoreNormalization(nn.Module):
            """Normalizes input to zero mean and unit variance along specified dimension"""
        ...
        \begin{minted}[
            ...
            class LogTransform(nn.Module):
                """Applies log transformation with offset for positive values: log(x + offset)"""
...
\begin{frame}[fragile]{Module Hierarchy with Layers within a Module}
...
\begin{frame}{Hooks in \texttt{nn.Module}}
    
    \begin{block}{What is a Hook?}

        In PyTorch, a hook is simply a function that you attach to either a Tensor or a \texttt{nn.Module}, and it gets executed automatically when the forward (not the same as forward function, but literally forward operation in the computation graph) or backward pass happens.
        
    \end{block}

    \begin{enumerate}
        \item Forward Hooks: Executed during forward pass
        \item Backward Hooks: Executed during backward pass
    \end{enumerate}

    A hook is a function with a specific signature. When we say a hook is executed, we are talking about this function being executed. Such functions are called \textbf{callbacks}.

    \vspace{5pt}
    They let us inspect, modify, or intercept the flow of data in our neural network without changing its source code.
\end{frame}

\begin{frame}[fragile]{Forward and Backward Hooks}
...
\begin{frame}[fragile]{Forward Hook Example}
          \begin{minted}[
                linenos,
                frame=lines,
                framesep=2mm,
...
\begin{frame}[fragile]{Backward Hook Example}
          \begin{minted}[
                linenos,
                frame=lines,
                framesep=2mm,
...
\begin{enumerate}
\item Can access input, output, and gradients of the module
\item Can modify the gradient (be careful with this one!)
\end{enumerate}
\end{frame}
