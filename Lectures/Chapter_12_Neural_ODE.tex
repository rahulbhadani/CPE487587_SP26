...
\begin{frame}{Neural ODEs in the Context of Discrete Dynamical Systems}
\begin{columns}
...
\[
  \hbf_{t+1} = \hbf_{t} + f(\hbf_{t},\,\theta_t), \quad t = 0, 1, \ldots, T-1
\]
\begin{itemize}
  \item $\hbf_t \in \Rbb^d$: hidden state at layer $t$
...
Euler's method for $\frac{d\hbf}{dt} = f(\hbf,t)$ with step $\Delta t = 1$:
\[
  \hbf(t + \Delta t) \approx \hbf(t) + \Delta t \cdot f\!\left(\hbf(t), t\right)
\]
\end{tblockFrost}
...
\begin{tikzpicture}[scale=0.9, node distance=0.8cm and 1.2cm, every node/.style={font=\scriptsize}]
  ...
  \node[draw, fill=gray!10, rounded corners, minimum width=1.5cm, minimum height=0.5cm, below=0.7cm of add] (out) {Output $\hbf_{t+1}$};
  \draw[->, thick] (in) -- (w1);
  \draw[->, thick] (w1) -- (relu);
  \draw[->, thick] (relu) -- (w2);
  \draw[->, thick] (w2) -- (add);
  ...
  \node[right=2.0cm of relu, text=DarkRed, font=\footnotesize] {$f(\hbf_t, \theta_t)$};
\end{tikzpicture}
\end{columns}
\end{frame}
...
\[
  \hbf_{t+1} = \hbf_t + \Delta t\, f(\hbf_t,\, t,\, \theta)
\]
converges to the \textbf{continuous ODE}:
\[
  \boxed{\frac{d\hbf(t)}{dt} = f\!\left(\hbf(t),\, t,\, \theta\right)}
\]
\end{tblockFrost}
\begin{columns}
...
\[
  \hbf_T = \hbf_0 + \sum_{t=0}^{T-1} f(\hbf_t, \theta_t)
\]
\end{block}
\column{0.48\textwidth}
\begin{block}{Continuous: Neural ODE}
\[
  \hbf(T) = \hbf(0) + \int_0^T f\!\left(\hbf(t), t, \theta\right) dt
\]
\end{block}
\end{columns}
\vspace{6pt}
...
Given dynamics $f$ and initial condition $\hbf(t_0) = \hbf_0$, find $\hbf(t)$:
\[
  \frac{d\hbf}{dt} = f(\hbf(t), t), \quad \hbf(t_0) = \hbf_0
\]
\textbf{Solution} (via Picard–Lindelöf):
\[
  \hbf(t) = \hbf_0 + \int_{t_0}^{t} f(\hbf(s), s)\, ds
\]
\end{tblockFrost}
...
\node[font=\tiny, red] at (axis cs:0.25, 0.9) {$\hbf_0$};
\end{axis}
\end{tikzpicture}
\end{columns}
\end{frame}
...
\vspace{-20pt}
\begin{align*}
  \hbf_{t+h} \approx \hbf_t + h\, f(\hbf_t, t)
\end{align*}
...
\begin{align*}
  k_1 &= f(\hbf_t,\, t)\\
  k_2 &= f\!\left(\hbf_t + \tfrac{h}{2}k_1,\, t+\tfrac{h}{2}\right)\\
  k_3 &= f\!\left(\hbf_t + \tfrac{h}{2}k_2,\, t+\tfrac{h}{2}\right)\\
  k_4 &= f(\hbf_t + h\, k_3,\, t+h)\\
  \hbf_{t+h} &= \hbf_t + \tfrac{h}{6}(k_1 + 2k_2 + 2k_3 + k_4)
\end{align*}
$\mathcal{O}(h^4)$ error per step.
\end{tblockWing}
\column{0.46\textwidth}
\begin{golden}{Adaptive Solvers (Dopri5)}
\begin{itemize}
...
\begin{frame}{Neural ODEs: The Core Idea}
\scriptsize
...
\[
  \frac{d\hbf(t)}{dt} = f_\theta\!\left(\hbf(t),\, t\right), \qquad \hbf(t_0) = \hbf_0
\]
The output at time $t_1$ is computed by \textbf{numerically integrating} the ODE:
\[
  \hbf(t_1) = \hbf(t_0) + \int_{t_0}^{t_1} f_\theta\!\left(\hbf(t), t\right) dt
  \;=\; \texttt{ODESolve}(f_\theta,\, \hbf_0,\, t_0,\, t_1)
\]
\end{tblockFrost}
\vspace{4pt}
\begin{columns}
...
\begin{enumerate}
  \item Set initial state $\hbf(t_0)$ (e.g., encoder output)
  \item Call ODE solver with $f_\theta$
  \item Obtain $\hbf(t_1)$ as the model output
...
\begin{tikzpicture}[scale=0.6, node distance=0.6cm and 0.4cm, every node/.style={font=\small}]
  ...
  % h0
  \node[draw, fill=green!15, rounded corners, minimum width=1.0cm, minimum height=0.7cm, right=1.5cm of enc] (h0) {$\hbf(t_0)$};
  % ODE block
  ...
  \node[font=\scriptsize] at (ode.center) {$\frac{d\hbf}{dt}=f_\theta(\hbf,t)$};
  \node[font=\scriptsize, below=0pt] at (ode.south) {\texttt{ODESolve}};
  % h1
  \node[draw, fill=green!15, rounded corners, minimum width=1.4cm, minimum height=0.7cm, right=1.5cm of ode] (h1) {$\hbf(t_1)$};
  % Output
...
$$
    \frac{d\hbf}{dt} = f_\theta(\hbf(t), t)
$$
  \item \textbf{Fully connected layer}: map $\hbf(t_1) \to$ class logits
\end{enumerate}
\end{tblockWing}
\vspace{4pt}

\column{0.44\textwidth}
\begin{tblockWing}{Why the ODE block is invertible?}
Given $\hbf(t_1)$, integrate the ODE \textbf{backwards} in time from $t_1$ to $t_0$:
\[
  \hbf(t_0) = \hbf(t_1) + \int_{t_1}^{t_0} f_\theta(\hbf(t), t)\, dt
\]
...
Define the \textbf{adjoint} $\abf(t) = \frac{\partial \mathcal{L}}{\partial \hbf(t)}$.
\begin{itemize}
  \item $\abf(t_1) = \frac{\partial \mathcal{L}}{\partial \hbf(t_1)}$ (from loss at output)
  \item $\abf(t)$ evolves \textbf{backwards} via its own ODE:
\end{itemize}
\[
  \frac{d\abf(t)}{dt} = -\abf(t)^\T \frac{\partial f_\theta(\hbf(t), t, \theta)}{\partial \hbf(t)}
\]
\end{block}
\vspace{4pt}
\begin{alertblock}{Memory Advantage}
The adjoint ODE is solved \textbf{backwards in time}, recomputing $\hbf(t)$ on the fly. No need to store forward trajectory. Memory cost: $\mathcal{O}(1)$ (constant w.r.t.\ solver steps)!
\end{alertblock}
\end{frame}
...
\[
  \frac{d}{dt}\begin{bmatrix}\hbf(t)\\ \abf(t)\\ \frac{d\mathcal{L}}{d\theta}\end{bmatrix}
  = \begin{bmatrix}
      f_\theta(\hbf, t)\\
      -\abf^\T \frac{\partial f_\theta}{\partial \hbf}\\
      -\abf^\T \frac{\partial f_\theta}{\partial \theta}
    \end{bmatrix}
\]
with initial conditions at $t_1$:
\[
  \hbf(t_1),\quad \abf(t_1) = \frac{\partial \mathcal{L}}{\partial \hbf(t_1)}, \quad \frac{d\mathcal{L}}{d\theta}\bigg|_{t_1} = \mathbf{0}
\]
\end{tblockFrost}

\begin{block}{After integrating to $t_0$:}
\begin{itemize}
  \item $\abf(t_0) = \frac{\partial \mathcal{L}}{\partial \hbf(t_0)}$ \quad (gradient w.r.t.\ initial state)
...
\SetAlgoLined
\KwIn{Parameters $\theta$, initial state $\hbf_0$, loss $\mathcal{L}$, times $t_0 < t_1$}
\KwOut{Gradients $\frac{d\mathcal{L}}{d\theta}$, $\frac{d\mathcal{L}}{d\hbf_0}$}
\BlankLine
\textbf{Forward pass:}\\
$\hbf(t_1) \leftarrow \texttt{ODESolve}(f_\theta, \hbf_0, t_0, t_1)$\;
Compute $\mathcal{L}(\hbf(t_1))$\;
\BlankLine
\textbf{Backward pass (Adjoint):}\\
Initialize $\abf(t_1) = \frac{\partial \mathcal{L}}{\partial \hbf(t_1)}$,\; $\gbf_\theta = \mathbf{0}$\;
Solve augmented ODE from $t_1$ to $t_0$:
$[\hbf(t_0),\, \abf(t_0),\, \gbf_\theta] \leftarrow \texttt{ODESolve}(\text{aug}, [\hbf_1, \abf_1, \mathbf{0}], t_1, t_0)$\;
\Return $\frac{d\mathcal{L}}{d\hbf_0} = \abf(t_0)$,\;\; $\frac{d\mathcal{L}}{d\theta} = \gbf_\theta$\;
\end{algorithm}
\vspace{4pt}
\begin{alertblock}{}
...
\[
  \log p_x(\xbf) = \log p_z(\zbf) - \log \left|\det \frac{\partial g}{\partial \zbf}\right|
\]
\end{tblockFrost}
\vspace{4pt}
...
\[
  \frac{d \log p(\zbf(t))}{dt} = -\operatorname{tr}\!\left(\frac{\partial f_\theta}{\partial \zbf(t)}\right)
\]
...
\[
  \tilde{\hbf}(t_0) = \begin{bmatrix} \hbf(t_0) \\ \mathbf{0}_p \end{bmatrix} \in \Rbb^{d+p}
\]
...
\begin{tblockFrost}{}
\begin{enumerate}
  \item \textbf{Neural ODEs} replace discrete layer stacks with a continuous ODE: $\frac{d\hbf}{dt} = f_\theta(\hbf, t)$
...
\begin{block}{}
\begin{itemize}
  \item Chen et al.\ (2018) — \textit{Neural Ordinary Differential Equations}, NeurIPS
  \item Grathwohl et al.\ (2018) — \textit{FFJORD}, ICLR 2019
  \item Rubanova et al.\ (2019) — \textit{Latent ODEs}, NeurIPS
  \item Kidger et al.\ (2020) — \textit{Neural CDEs}
  \item Song et al.\ (2021) — \textit{Score-Based Generative Modeling through SDEs}
\end{itemize}
\end{block}
\column{0.48\textwidth}
\begin{tblockFrost}{}
