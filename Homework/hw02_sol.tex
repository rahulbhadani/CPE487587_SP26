...
\newcommand{\hw}{Homework 2: Training a Neural Network}
...
\item
\begin{align*}
\nabla_\xbf y = [ \pdv{y}{x_1} ,  \pdv{y}{x_2}, \cdots,  \pdv{y}{x_n}] = [z, z, \cdots, z]
\end{align*}
The dimension is $1 \times n$.
...
Let $u = x^3$, then $ y = \ln( sin(u)^2 ) $

Let $v = sin(u)$, then $y = \ln(v^2)$

Let $w   = v^2$, then $y = \ln(w)$.

Hence, by the chain rule:

\begin{align*}
\frac{dy}{dx} & = \pdv{y}{w} \times \pdv{w}{v} \times \pdv{v}{u} \pdv{u}{x} = \frac{1}{w} \times 2v \times \cos(u) \times 3 x^2\\
& = \frac{1}{sin(x^3)^2} \times 2 \sin(x^3) \cos(x^3) 3x^2
\end{align*}
...
where $X$ has dimension $n \times d$, $W$ has dimension  $d\times n$ and $b$ can be either a column vector with dimension $d\times 1$ or a scalar and elementwise addition will be used.

\item 

\begin{align*}
C = \frac{1}{n} \vbf^\top \vbf
\end{align*} 
...
$W_1$ becomes one-dimensional vector under two circumstances:

\begin{enumerate}
\item If we only have only linear layer and there won't be any subsequent linear layer or affine transformation, then in that case, we won't need a matrix $W_1$ because we are not looking to change the dimension of features.
\item If we only consider one sample at a time, weight matrix is just one dimensional vector.
\end{enumerate}
...
\begin{figure}[htpb]
\centering
\includegraphics[width=0.57\linewidth, trim={0cm 0.0cm 0cm 0.0cm},clip]{figures/streamline_cpe487587_hw2.pdf}
...
Critical point is approximately at $(0, 0)$, based on the color graph, it seesm that it represents maximum.
...
Consider only three points for the example.

$$Loss = f(x_1, y_1) + f(x_2, y_2) + f(x_3, y_3)$$

$$\frac{\partial Loss}{\partial x_1} = \frac{\partial f}{\partial x_1} \quad \text{(only depends on the first set of points.)}$$
$$\frac{\partial Loss}{\partial y_1} = \frac{\partial f}{\partial y_1}$$

$$\frac{\partial Loss}{\partial x_2} = \frac{\partial f}{\partial x_2} \quad \text{(only depends on the second set of points.)}$$
$$\frac{\partial Loss}{\partial y_2} = \frac{\partial f}{\partial y_2}$$

$$\frac{\partial Loss}{\partial x_3} = \frac{\partial f}{\partial x_3} \quad \text{(only depends on the third set of points.)}$$
$$\frac{\partial Loss}{\partial y_3} = \frac{\partial f}{\partial y_3}$$

\section*{Practice}

Solution through Github repo.
